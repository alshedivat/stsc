function Sout = learn_coefficients(B, X, gamma, L, Sinit)
% The feature-sign search algorithm
% L1-regularized least squares problem solver
%
% This code solves the following problem:
%
%   minimize_s 0.5*(||xi-B*si||^2 + si'si + 2*si(sigma(Lij*sj))
%              + gamma*||si||_1
% X : data matrix
% B : basis matrix
...
% Modified by Maruan Al-Shedivat
% =========================================================================
...
%     if mod(i, 100)==0, fprintf('.'); end %fprintf(1, 'l1ls_featuresign: %d/%d\r', i, size(X,2)); end
    if use_Sinit
        ...
        Sout(:,i) = ls_featuresign_sub (B, S, X(:,i), BtB, BtX(:,i), L, i, gamma, sinit);
    else
        Sout(:,i) = ls_featuresign_sub (B, Sout, X(:,i), BtB, BtX(:,i), L, i, gamma);
    end
end
...
function [s, fobj] = ls_featuresign_sub(B, S, x, BtB, Btx, L, i, gamma, sinit)
...
fobj = 0;
%fobj = fobj_featuresign(s, B, x, BtB, Btx, P, L, i, gamma);
...
for iter=1:ITERMAX
    ...
    grad = BtB*sparse(s)+L(i,i)*sparse(s)+P - Btx;
    theta = sign(s);
    ...
    while 1
        ...
        [s, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (s, B, x, BtB, Btx, P, L, i, theta, act, act_indx1, gamma);
...
function [s, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (s, B, x, BtB, Btx, P, L, idx, theta, act, act_indx1, gamma)
...
a = L(idx,idx)*ones(size(act_indx1));
%a = L(idx,idx)*eye(size(act_indx1,1));
% call matlab optimization solver..
s_new = (BtB2+diag(a)) \ ( Btx(act_indx1) - gamma.*theta2 -P(act_indx1)); % RR
...
if (sign(s_new) == sign(s2))
    ...
    s(act_indx1) = s_new;
    fobj = 0;
    %fobj = fobj_featuresign(s, B, x, BtB, Btx, P, L, idx, gamma);
    lsearch = 1;
    return;
end
...
a= 0.5*sum((B(:,act_indx1)*(s_new-s2)).^2)+0.5*L(idx,idx)*(s_new-s2)'*(s_new-s2);
b= (s_new-s2)'*(P(act_indx1)-Btx(act_indx1)) + (s2'*BtB2+ L(idx,idx)*s2')*(s_new-s2);
...
fobj_new = 0;
%fobj_new = fobj_featuresign(s, B, x, BtB, Btx, P, L, idx, gamma);
...
function [f, g] = fobj_featuresign(s, B, x, BtB, Btx, P, L, i, gamma)

f= 0.5*(norm(x-B*s)^2+L(i,i).*(s'*(s+2*P)));
f= f+ gamma*norm(s,1);

if nargout >1
    g= BtB*s+L(i,i)*s+P - Btx;
    g= g+ gamma*sign(s);
end
