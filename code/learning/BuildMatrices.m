function [L, MMD, GraphLaplacian] = BuildMatrices(TrainIn, Options, verbose)
% BUILDMATRICES Builds all the matrices necessary used in learning.
% =========================================================================

SampleNum = size(TrainIn.Features,2);

% Build domain transfer matrix (MMD matrix)
if strcmp(Options.TransMethod, 'EachClass')  % use for supervised only
    MMD = zeros(SampleNum);
    LabelSet = unique(TrainIn.Labels);
    for y = LabelSet
        Idx_y = find(TrainIn.Labels==y);
        Idx_ys = Idx_y(TrainIn.DomainIdx(Idx_y) == +1); % source
        N_ys = length(Idx_ys);
        Idx_yt = Idx_y(TrainIn.DomainIdx(Idx_y) == -1); % traget
        N_yt = length(Idx_yt);

        Pi = zeros(1,SampleNum);
        Pi(Idx_ys) = 1/N_ys;
        Pi(Idx_yt) = -1/N_yt;
        MMD = MMD + Pi'*Pi;
    end
else
    Pi    = TrainIn.DomainIdx;
    J     = find(TrainIn.DomainIdx == +1);
    Pi(J) = Pi(J) / size(Pi(J),2);
    J     = find(TrainIn.DomainIdx == -1);
    Pi(J) = Pi(J) / size(Pi(J),2);
    MMD = Pi'*Pi;
end
...
if verbose
    figure('name','MMD matrix'); imagesc(MMD); colorbar;
end

% Build the GraphMatrix
%[IDX, ~] = knnsearch(TrainIn.Features',TrainIn.Features','K',Options.p,...
%                     'Distance','euclidean','NSMethod','kdtree');
%GraphMatrix = zeros(SampleNum);
%for i = 1:SampleNum
%    GraphMatrix(i,IDX(i,:)) = 1;
%    GraphMatrix(IDX(i,:),i) = 1;
%end

W = constructW(TrainIn.Features');
DCol = full(sum(W,2));
D = spdiags(DCol,0,speye(size(W,1)));
GraphLaplacian = full(D - W);

if verbose
    figure('name','GraphL matrix'); imagesc(GraphLaplacian); colorbar;
end

% Add up matrices with weights
L = Options.f*MMD + Options.h*GraphLaplacian;
...
%EUDIST2 Efficiently Compute the Euclidean Distance Matrix by Exploiting
